...
publish_runtime() {
    ...
    dotnet publish "$PROJECT_PATH" \
        ...
        --runtime "$runtime" \
        --self-contained true \
        --output "$OUTPUT_BASE/$runtime" \
        /p:PublishSingleFile=true \
        /p:PublishTrimmed=false \
        /p:IncludeNativeLibrariesForSelfExtract=true
    
    echo "✓ Published $platform to $OUTPUT_BASE/$runtime"
}
...
echo "  - $OUTPUT_BASE/win-x64/azdo-wi-creator.exe"
echo "  - $OUTPUT_BASE/win-arm64/azdo-wi-creator.exe"
echo "  - $OUTPUT_BASE/osx-x64/azdo-wi-creator"
echo "  - $OUTPUT_BASE/osx-arm64/azdo-wi-creator"
